...
def my_account(request):
    ...
    return render(request, 'app/my_account.html',
                  {'name': name, 'email': email,'Age': Age, 'weight': weight, 'height': height, 'date': date,
...
def update(request):
    ...
    return render(request, 'app/my_account.html',
                  {'instance': instance, 'name': name, 'age': Age, 'email': email, 'hydration': Hydra, 'date': date})
...
def activity(request):
    if request.method == "POST":
        ...
        if 2 < time <= 3:
            hydration = hydration + 0.4
        elif 3 < time <= 4:
            hydration = hydration + 0.8
        elif 4 < time <= 5:
            hydration = hydration + 1.11
        elif time > 5:
            hydration = hydration + 1.18
